<template>
  <div class="w-full p-2 bg-white rounded-xl select-none">
    <!-- 搜索框 -->
    <div class="flex flex-row m-4">
      <SearchBox class="flex-auto" />

      <div
        @click="changeAdvancedSearchStatus(!showAdvancedSearch)"
        class="flex-initial ml-2 rounded-md py-2.5 px-3.5 shadow-lg text-pink-300 ring-2 ring-inset ring-pink-300 hover:bg-pink-500 hover:ring-pink-500 transition"
      >
        <ChevronUpIcon :class="advancedSearchBtnStyle" />
      </div>
    </div>
    ...
    <div v-show="showAdvancedSearch">
      <div class="grid grid-cols-3 gap-4 m-4">
        ...
        <div class="flex flex-row items-center">
          ...
          <ListBox class="flex-auto ml-2" :list-items="categories" />
        </div>
        <div class="flex flex-row items-center">
...
import { ArrowUpRightIcon, ChevronUpIcon } from '@heroicons/vue/20/solid';
...
// 展开属性及动画
const advancedSearchBtnStyle = ref('w-5 ease-in-out transition-all');
const showAdvancedSearch = ref(false);
function changeAdvancedSearchStatus(status: boolean) {
  if (status) {
    advancedSearchBtnStyle.value = 'w-5 rotate-180 ease-in-out transition-all';
  } else {
    advancedSearchBtnStyle.value = 'w-5 ease-in-out transition-all';
  }
  showAdvancedSearch.value = status;
}
</script>
